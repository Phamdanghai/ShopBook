...
import haipd.login.RegistrationCreateError;
import haipd.util.MyApplicationConstant.CreateAccountFeatures;
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (username.trim().length() < 6
                ...
                founErr = true;
                errors.setUsernameLengErr("Username is required from 6 to 30 chars");
            }
            if (password.trim().length() < 6
                    || password.trim().length() > 20) {
                founErr = true;
                errors.setPasswordLengErr("Password is required from 6 to 20 chars");
            } else if (!confirm.trim().equals(password.trim())) {
                founErr = true;
                errors.setCofirmNotMatchPassword("Confirm must match Password");
            }
            if (fullName.trim().length() < 2
                    || fullName.trim().length() > 50) {
                founErr = true;
                errors.setFullnameLengErr("Full name is required from 2 to 50 chars");
            }
            if (founErr) {
